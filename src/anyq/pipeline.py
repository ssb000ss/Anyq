...
from anyq.jobs.storage import Job, JobStatus, RedisJobStorage, Report
...
async def run_pipeline(
    ...
    try:
        ...
        key_phrases = TFIDFExtractor.extract_key_phrases(doc.full_text)
        samples = TFIDFExtractor.extract_sample_sentences(doc.full_text)
        ...
        await storage.delete_upload(job_id)
        ...
        log.exception("pipeline.failed", job_id=job_id, error=str(exc))
        await storage.update_job(
            job_id,
            status=JobStatus.failed,
